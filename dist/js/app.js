(() => {
    ...
    let bodyLockToggle = (delay = 1e3) => {
        if (document.documentElement.classList.contains("lock")) bodyUnlock(delay); else bodyLock(delay);
    };
    let bodyUnlock = (delay = 1e3) => {
    ...
    if (firstscreen) {
        ...
        function firstscreenHandleMmd3Change(e) {
            if (e.matches) {
                function setCoordinates() {
                    ...
                    const image = document.querySelector(".firstscreen__image img"), imageComputedStyle = window.getComputedStyle(image), coordinates = (imageComputedStyle.getPropertyValue("object-fit"), 
                    {}), imagePositions = imageComputedStyle.getPropertyValue("object-position").split(" "), horizontalPercentage = parseInt(imagePositions[0]) / 100, verticalPercentage = parseInt(imagePositions[1]) / 100, naturalRatio = image.naturalWidth / image.naturalHeight, visibleRatio = image.width / image.height, hotPots = document.querySelector(".firstscreen__points");
                    let koef = 0;
        ...
        function firstscreenHandleMmd2Change(e) {
            if (e.matches) ; else firstscreen.style.marginTop = 0 + "px";
        }
